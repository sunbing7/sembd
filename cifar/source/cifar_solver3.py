...
class solver:
    ...
    def solve_fp(self, gen):
        ...
        out = np.append(np.arange(0, len(perm_predict_avg), 1, dtype=int).reshape(-1, 1),
                        perm_predict_avg.reshape(-1, 1), axis=1)
    ...
    def find_clusters_act(self, prefix=''):
        ...
        for cur_class in self.classes:
            ...
            for cur_layer in self.layer:
                ...
                hidden_test_ = np.loadtxt(
                    RESULT_DIR + "test_act_" + "c" + str(cur_class) + "_layer_" + str(cur_layer) + ".txt")
                ...
                np.savetxt(
                    RESULT_DIR + prefix + "act_kmeans" + "c" + str(cur_class) + "_layer_" + str(cur_layer) + ".txt",
                    labels, fmt="%s")
                print('class: {} layer: {} cntroid: {}'.format(cur_class, cur_layer, centroid.transpose()))
                layer_label.append(labels)

                # plotting
                plt.scatter(*np.transpose(ar.reshape(-1, 1)),
                            *np.transpose(np.arange(0, len(hidden_test_), 1, dtype=int).reshape(-1, 1)), c=labels)
    ...
    def find_clusters(self, prefix=''):
        ...
        for cur_class in self.classes:
            ...
            for cur_layer in self.layer:
                ...
                temp.sort()
                layer_centroid = [*layer_centroid, *temp.tolist()]
                np.savetxt(RESULT_DIR + prefix + "kmeans" + "c" + str(cur_class) + "_layer_" + str(cur_layer) + ".txt",
                           labels, fmt="%s")
                layer_label.append(labels)

                # plotting
                plt.scatter(*np.transpose(ar.reshape(-1, 1)), *np.transpose(hidden_test_[:, 0].reshape(-1, 1)),
                            c=labels)
                title = "class: {}, layer: {}, number of clusters: {}".format(cur_class, cur_layer, k_val)
                plt.title(title)
                plt.xlim(0., 1.)

...
def custom_loss(y_true, y_pred):
    ...
    loss3 = 1.0 - K.square(y_pred[:, 0] - y_pred[:, 6])
    loss4 = 1.0 - K.square(y_pred[:, 4] - y_pred[:, 2])
    loss2 = K.sum(loss2)
    loss3 = K.sum(loss3)
    loss4 = K.sum(loss4)
...
def remove_backdoor():
    rep_neuron = [1,3,9,13,16,17,18,29,31,35,49,51,52,54,59,63,65,67,70,88,97,100,107,109,122,124,125,129,134,137,140,157,159,161,166,172,174,183,184,186,193,194,197,200,203,204,210,211,212,237,240,241,244,246,248,254,261,264,266,272,278,285,290,300,303,306,311,320,325,332,337,345,349,351,356,361,376,381,383,385,402,403,406,410,412,415,418,429,430,431,439,441,442,449,452,473,474,475,476,482,487,490,501,505]
...
def remove_backdoor_rq3():
    rep_neuron = np.unique((np.random.rand(104) * 512).astype(int))

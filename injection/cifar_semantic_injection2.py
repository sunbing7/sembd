...
SBG_TST = [3976,4543,4607,4633,6832]
...
def load_dataset_repair(data_file=('%s/%s' % (DATA_DIR, DATA_FILE))):
    ...
    x_train_mix = np.concatenate((x_clean[int(len(x_clean) * 0.1):], x_trigs), axis=0)
    y_train_mix = np.concatenate((y_clean[int(len(y_clean) * 0.1):], y_trigs), axis=0)

    x_train_c = x_clean[int(len(x_clean) * DATA_SPLIT):int(len(x_clean) * 0.1)]
    y_train_c = y_clean[int(len(y_clean) * DATA_SPLIT):int(len(x_clean) * 0.1)]

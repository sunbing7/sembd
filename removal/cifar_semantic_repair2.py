...
def load_dataset_repair(data_file=('%s/%s' % (DATA_DIR, DATA_FILE)), ae_known=False, is_real=False):
    ...
    if ae_known:
        ...
        x_train_mix = x_trigs#np.concatenate((x_clean[int(len(x_clean) * 0.8):], x_trigs), axis=0)
        y_train_mix = y_trigs#np.concatenate((y_clean[int(len(y_clean) * 0.8):], y_trigs), axis=0)
...
def custom_loss(y_true, y_pred):
    ...
    loss5 = 1.0 - K.square(y_pred[:, 8] - y_pred[:, 0])
    loss2 = K.sum(loss2)
    loss3 = K.sum(loss3)
    loss4 = K.sum(loss4)
    loss5 = K.sum(loss5)
...
def remove_backdoor(is_real=False):
    ...
    x_train_c, y_train_c, x_test_c, y_test_c, x_train_adv, y_train_adv, x_test_adv, y_test_adv, x_clean, y_clean = load_dataset_repair()

    # build generators
    clean_gen = build_data_loader_aug(x_clean, y_clean)
    ...
    model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])
    model.fit_generator(clean_gen, steps_per_epoch=50, epochs=1, verbose=0,
                        callbacks=[cb])
...
def remove_backdoor_rq3():
    ...
    rep_neuron = np.unique((np.random.rand(304) * 512).astype(int))
